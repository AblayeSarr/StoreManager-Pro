...
<?php foreach ($sales as $sale): ?>
    <tr>
        ...
        <td>
            <?= date('d M H:i', strtotime($sale['date'])) ?>
        </td>
        <td style="font-weight: 700;">
            <?= $sale['client_nom'] ?>
        </td>
        <td style="font-weight: 800; color: var(--accent);">
            <?= number_format($sale['montant_total'], 0, ',', ' ') ?> F
        </td>
        <td>
            <?php if ($sale['statut'] === 'CREDIT'): ?>
                    ...
                    <div id="dash-left-panel-ruptures" style="display: none;">
                        ...
                        <div style="display: flex; flex-direction: column; gap: 14px;">
                                    ...
                                    <div style="display: flex; justify-content: space-between; align-items: center;">
                                        ...
                                        <button type="button" class="btn-quick-action" onclick="toggleDetails('supply-product-drawer-2')" style="border-color: var(--accent); color: var(--accent); background: rgba(45, 212, 191, 0.05);">
                                            Approvisionner
                                        </button>
                                    </div>
                                    ...
                                    <div class="details-drawer" id="supply-product-drawer-2" style="margin-top: 10px; padding: 10px;">
                                        ...
                                        <form method="GET" action="#" onsubmit="event.preventDefault(); alert('Action enregistrée (mode démonstration HTML/CSS)');" action="" style="display: grid; grid-template-columns: 1.5fr 1fr 1fr auto; gap: 8px; align-items: flex-end;">
                                            ...
                                            <div>
    <label style="font-size: 9px; color: var(--text-muted); display: block; margin-bottom: 2px;">
        Fournisseur
    </label>
    ...
    >
        ...
        <?php endforeach; ?>
                                    ...
                                    <div class="details-drawer" id="supply-product-drawer-3" style="margin-top: 10px; padding: 10px;">
                                        <div style="font-weight: 700; font-size: 11px; color: var(--accent); margin-bottom: 6px;">Commande d'Approvisionnement Rapide :</div>
                                        <form method="POST" action="" style="display: grid; grid-template-columns: 1.5fr 1fr 1fr auto; gap: 8px; align-items: flex-end;">
                                            <input type="hidden" name="action" value="quick_supply_product">
...
<?php foreach ($products as $product): ?>
      <tr data-product-name="<?= htmlspecialchars(strtolower($product['nom'])) ?>">
        <td style="font-weight: 700;">
            <?= $product['nom'] ?>
        </td>
        <td>
...
<?php foreach ($clients as $client): ?>
    <tr>
        ...
        <td style="font-weight: 700; color: var(--accent);">
            <?= number_format($client['limite_credit'], 0, ',', ' ') ?> F
        </td>
    </tr>
<?php endforeach; ?>
</tbody>
