...
require_once __DIR__ . '/../../src/Repositories/DetteRepository.php';
require_once __DIR__ . '/../../src/Services/DebtService.php';
                ...
                <?php else: ?>
                    ...
                    <?php foreach ($dettes as $dette): ?>
                        ...
                        $statutLabel = match ($dette['statut']) {
                            'en_cours' => 'NON SOLDÉE',
                            'partiellement_remboursee' => 'PARTIELLE',
                            'soldee' => 'SOLDÉE',
                            default => strtoupper($dette['statut'])
                        };
                        ...
                        <tr id="debt-row-<?= $id ?>" data-client-name="<?= htmlspecialchars(strtolower($client . ' ' . ($dette['telephone'] ?? ''))) ?>">
                            ...
                            <td style="display:flex;gap:6px;flex-wrap:wrap;">
                                <button type="button" class="btn-quick-action" onclick="toggleDetails('debt-details-<?= $id ?>')">
                                    💳 Paiements
                                </button>

                                <?php if ($reste > 0): ?>
                                    <button type="button" class="btn-quick-action" style="border-color:var(--warning);color:var(--warning);" onclick="toggleDetails('debt-repay-<?= $id ?>')">
                                        Rembourser
                                    </button>
                                <?php endif; ?>
                            </td>
                        ...
                        <tr>
                            <td colspan="8" style="padding:0;border:none;">
                                        ...
                                        <tbody>
                                            ...
                                            <?php else: ?>
                                                <?php foreach ($dette['remboursements'] as $remboursement): ?>
                                                    <tr>
                                                        <td><?= htmlspecialchars($remboursement['date'] ?? '-') ?></td>
                                                        <td style="font-weight:700;color:var(--success);">
                                ...
                                <!-- Remboursement -->
                                <?php if ($reste > 0): ?>
                                    <div class="details-drawer" id="debt-repay-<?= $id ?>" style="border:1px solid rgba(45,212,191,.25);background:#0b0f19;border-radius:14px;padding:18px 20px;max-width:850px;margin:12px 0;">
                                        ...
                                        <div style="display:flex;gap:8px;align-items:center;margin-bottom:16px;">
                                            ...
                                            <button type="button" onclick="setRepayAmount(<?= $id ?>,<?= $reste ?>)" style="background:rgba(45,212,191,.1);border:1px solid var(--accent);color:var(--accent);font-size:10px;font-weight:700;padding:4px 10px;border-radius:6px;cursor:pointer;">
                                                Tout solder
                                            </button>

                                            <button type="button" onclick="setRepayAmount(<?= $id ?>,<?= floor($reste / 2) ?>)" style="background:rgba(255,255,255,.04);border:1px solid var(--border-color);color:var(--text-main);font-size:10px;font-weight:700;padding:4px 10px;border-radius:6px;cursor:pointer;">
                                                50%
                                            </button>
                                        </div>
                                        ...
                                        <form method="POST" action="" style="display:flex;gap:16px;align-items:flex-end;flex-wrap:wrap;">
                                            ...
                                            <div style="flex:1;min-width:200px;">
                                                ...
                                                <input type="number" name="montant_verse" id="repay-input-<?= $id ?>" class="form-control" min="1" max="<?= $reste ?>" value="<?= $reste ?>" required style="font-size:13px;font-weight:700;padding:10px 12px;background:#0b0f19;color:white;width:100%;">
                                            </div>
                                            ...
                                            <div style="flex:1;min-width:200px;">
                                                ...
                                                <select name="mode_paiement" class="form-control" required style="font-size:13px;font-weight:600;padding:10px 12px;background:#0b0f19;color:white;width:100%;">
                                            ...
                                            <button type="submit" class="btn-submit btn-success" style="padding:11px 24px;font-size:12px;font-weight:800;border-radius:10px;height:42px;">
                                                ✓ Enregistrer
                                            </button>
                                        </form>
                                    </div>
